...
final class TermuxInstaller {
    ...
    private static void setupMenuScripts(Context context) {
        ...
        depsSb.append("# Install clang, libc++, and ndk-sysroot via download-only + manual extract\n");
        depsSb.append("# apt-get install -y fails on Android 16 (dpkg maintainer scripts blocked)\n");
        depsSb.append("# Using -d (download-only) bypasses configure entirely; we extract .debs manually\n");
        depsSb.append("echo \"  Downloading clang and dependencies (this may take a while)...\"\n");
        depsSb.append("apt-get install -y -d clang libc++ ndk-sysroot 2>&1 | tail -5\n");
        depsSb.append("rc=${PIPESTATUS[0]}\n");
        depsSb.append("if [ $rc -ne 0 ]; then\n");
        depsSb.append("    echo \"  Download phase may have partial errors (exit code $rc)\"\n");
        depsSb.append("fi\n");
        depsSb.append("# Extract all cached .deb files manually (bypasses dpkg configure)\n");
        depsSb.append("CACHE=$PREFIX/var/cache/apt/archives\n");
        depsSb.append("found=0\n");
        depsSb.append("for deb in \"$CACHE\"/*.deb; do\n");
        depsSb.append("    [ -f \"$deb\" ] || continue\n");
        depsSb.append("    echo \"  Extracting $(basename $deb)...\"\n");
        depsSb.append("    python3 $PREFIX/bin/termux-install-deb.py \"$deb\" 2>/dev/null && found=$((found+1)) || echo \"  WARN: $(basename $deb) extract failed\"\n");
        depsSb.append("done\n");
        depsSb.append("rm -f \"$CACHE\"/*.deb 2>/dev/null || true\n");
        depsSb.append("echo \"  Extracted $found packages\"\n");
        depsSb.append("# Verify clang can compile C code\n");
        depsSb.append("echo 'int main(){}' | clang -x c - -o /dev/null 2>/dev/null && echo \"  clang C test: OK\" || echo \"  WARNING: clang C test failed\"\n");
        depsSb.append("# Verify clang++ can compile C++ code (stdlib.h error was in audiofile.cpp)\n");
        depsSb.append("echo 'int main(){}' | clang++ -x c++ - -o /dev/null 2>/dev/null && echo \"  clang++ test: OK\" || echo \"  WARNING: clang++ test failed - check libc++ headers\"\n");
        depsSb.append("\n");
        ...
        depsSb.append("# Create g++/gcc symlinks (some Makefiles like audiofile use these)\n");
