...
final class TermuxInstaller {
    ...
    private static void setupMenuScripts(Context context) {
        ...
        depsSb.append("    dpkg-deb --extract \"$savedir/$deb\" $TMPDIR/py_install 2>/dev/null\n");
        depsSb.append("    # Copy files with path remapping (cp -a handles symlinks + permissions)\n");
        depsSb.append("    srcdir=\"$TMPDIR/py_install/data/data/com.termux/files/usr\"\n");
        depsSb.append("    if [ -d \"$srcdir\" ]; then\n");
        depsSb.append("        cp -a \"$srcdir\"/* $PREFIX/ 2>/dev/null || true\n");
        depsSb.append("    fi\n");
