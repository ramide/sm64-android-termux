...
final class TermuxInstaller {
    ...
    private static void setupMenuScripts(Context context) {
        ...
        pySb.append("\n");
        pySb.append("# Use ar to extract data.tar.xz (needs LD_LIBRARY_PATH for libLLVM)\n");
        pySb.append("env = os.environ.copy()\n");
        pySb.append("subprocess.run(['ar', 'x', deb], capture_output=True, env=env)\n");
        pySb.append("\n");
        pySb.append("for tf in ['data.tar.xz', 'data.tar.gz', 'data.tar.bz2']:\n");
        pySb.append("    if not os.path.exists(tf):\n");
        pySb.append("        continue\n");
        pySb.append("    with tarfile.open(tf, 'r:*') as tar:\n");
        ...
        depsSb.append("    if java -version >/dev/null 2>&1 && apksigner --version >/dev/null 2>&1; then\n");
        depsSb.append("        echo \"Build dependencies already installed. Skip.\"\n");
        depsSb.append("        exit 0\n");
        depsSb.append("    fi\n");
        depsSb.append("    echo \"Sentinel found but java/apksigner broken, reinstalling...\"\n");
        depsSb.append("fi\n");
        depsSb.append("\n");
        ...
        depsSb.append("echo \"[1/5] Updating apt cache...\"\n");
        depsSb.append("apt-get update 2>/dev/null || echo \"  Warning: apt update failed, continuing with cached data\"\n");
        depsSb.append("\n");
        depsSb.append("# ── 2. Install Java + apksigner + deps ──\n");
        depsSb.append("echo \"[2/5] Installing Java 17 and apksigner...\"\n");
        depsSb.append("# Need ar to extract debs; binutils should be in bootstrap\n");
        depsSb.append("if ! ar --version >/dev/null 2>&1; then\n");
        depsSb.append("    echo \"  ar not available, installing binutils...\"\n");
        depsSb.append("    install_deb binutils || exit 1\n");
        depsSb.append("fi\n");
        depsSb.append("\n");
        ...
        depsSb.append("\n");
        depsSb.append("# ── 3. Create java wrapper ──\n");
        depsSb.append("echo \"[3/5] Creating java wrapper...\"\n");
        ...
        depsSb.append("\n");
        depsSb.append("# ── 4. Download SDL2 headers ──\n");
        depsSb.append("echo \"[4/5] Downloading SDL2 and GLES2 headers...\"\n");
        ...
        depsSb.append("  wget -q -O \"$SDLDIR/$hdr\" \"$SDL_BASE/$hdr\" 2>/dev/null || echo \"  WARN: $hdr failed\"\n");
        depsSb.append("done\n");
        ...
        depsSb.append("wget -q -O \"$PREFIX/include/GLES2/gl2platform.h\" \"$GL_BASE/GLES2/gl2platform.h\"\n");
        depsSb.append("wget -q -O \"$PREFIX/include/GLES2/gl2.h\" \"$GL_BASE/GLES2/gl2.h\"\n");
        depsSb.append("wget -q -O \"$PREFIX/include/GLES2/gl2ext.h\" \"$GL_BASE/GLES2/gl2ext.h\"\n");
        depsSb.append("wget -q -O \"$PREFIX/include/KHR/khrplatform.h\" \"$GL_BASE/KHR/khrplatform.h\"\n");
        depsSb.append("echo \"  GLES2 headers installed\"\n");
        depsSb.append("\n");
        depsSb.append("# ── 5. Create as wrapper (clang integrated assembler) ──\n");
        depsSb.append("echo \"[5/5] Creating as wrapper...\"\n");
