...
final class TermuxInstaller {
    ...
    private static void setupMenuScripts(Context context) {
        ...
        depsSb.append("    local savedir=$(pwd)\n");
        ...
        depsSb.append("    local deb=$(ls python_*.deb 2>/dev/null | head -1)\n");
        depsSb.append("    [ -z \"$deb\" ] && { echo \"  No python deb found\"; exit 1; }\n");
        depsSb.append("    echo \"  Extracting python...\"\n");
        depsSb.append("    mkdir -p /tmp/py_install\n");
        depsSb.append("    cd /tmp/py_install\n");
        depsSb.append("    dpkg-deb --fsys-tarfile \"$savedir/$deb\" | tar -xf - 2>/dev/null\n");
        depsSb.append("    cd /\n");
        depsSb.append("    # Copy files with path remapping\n");
        depsSb.append("    local srcdir=\"/tmp/py_install/data/data/com.termux/files/usr\"\n");
        depsSb.append("    if [ -d \"$srcdir\" ]; then\n");
        depsSb.append("        find \"$srcdir\" -type f | while read f; do\n");
        depsSb.append("            local rel=\"${f#$srcdir/}\"\n");
        depsSb.append("            [ -z \"$rel\" ] && continue\n");
        depsSb.append("            mkdir -p \"$PREFIX/$(dirname \"$rel\")\" 2>/dev/null\n");
        depsSb.append("            cp \"$f\" \"$PREFIX/$rel\" 2>/dev/null || true\n");
        depsSb.append("        done\n");
        depsSb.append("        find \"$srcdir\" -type l | while read f; do\n");
        depsSb.append("            local rel=\"${f#$srcdir/}\"\n");
        depsSb.append("            local tgt=$(readlink \"$f\")\n");
        depsSb.append("            ln -sf \"$tgt\" \"$PREFIX/$rel\" 2>/dev/null || true\n");
        depsSb.append("        done\n");
        depsSb.append("    fi\n");
        depsSb.append("    rm -rf /tmp/py_install \"$savedir/$deb\"\n");
